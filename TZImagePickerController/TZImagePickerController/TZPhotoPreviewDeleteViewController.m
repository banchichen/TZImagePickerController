//
//  TZPhotoPreviewDeleteViewController.m
...
@interface TZPhotoPreviewDeleteViewController () <UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate> 
{
...
@implementation TZPhotoPreviewDeleteViewController
...
//    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    ...
    _deleteButton.titleLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:17];
    [_deleteButton setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
//    [_deleteButton setBackgroundImage:[UIImage imageNamedFromMyBundle:tzImagePickerVc.photoDefImageName] forState:UIControlStateNormal];
//    [_deleteButton setBackgroundImage:[UIImage imageNamedFromMyBundle:tzImagePickerVc.photoSelImageName] forState:UIControlStateSelected];
...
//    _titleLabel.text = [NSString stringWithFormat:@"%lu/%lu", _currentIndex+1, (unsigned long)tzImagePickerVc.selectedAssets.count];
...
//    TZImagePickerController *_tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    ...
    _deleteButton.frame = CGRectMake(self.view.tz_width - 54, 17, 27, 27);
    _titleLabel.bounds = CGRectMake(0, 0, 100, 100);
...
//    }
    model.isSelected = !deleteButton.isSelected;

    [_collectionView reloadData];
    if (_currentIndex != 0) {
        [_collectionView setContentOffset:CGPointMake((self.view.tz_width + 20) * (_currentIndex-1), 0) animated:NO];
    } else if (_currentIndex == 0 && _tzImagePickerVc.selectedAssets.count) {
        [_collectionView setContentOffset:CGPointMake((self.view.tz_width + 20) * _currentIndex, 0) animated:NO];
        [self refreshNaviBarAndBottomBarState];
    } else if (_currentIndex == 0 && _tzImagePickerVc.selectedAssets.count == 0){
        [self refreshNaviBarAndBottomBarState];
    }
    
//    if (model.isSelected) {
//        [UIView showOscillatoryAnimationWithLayer:deleteButton.imageView.layer type:TZOscillatoryAnimationToBigger];
//    }
...
//    if (_progress > 0 && _progress < 1 && (_deleteButton.isSelected || !_tzImagePickerVc.selectedModels.count )) {
...
- (void)refreshNaviBarAndBottomBarState {
    TZImagePickerController *_tzImagePickerVc = (TZImagePickerController *)self.navigationController;
//    TZAssetModel *model = _models[_currentIndex];
//    _deleteButton.selected = model.isSelected;
//    if (_deleteButton.isSelected) {
...
//                [_deleteButton setTitle:[NSString stringWithFormat:@"%ld", idx + 1] forState:UIControlStateNormal];
//                *stop = YES;
//            }
//        }];
//    } else {
...
//    }
    ...
    if (_tzImagePickerVc.selectedModels.count != 0) {
        ...
        _titleLabel.text = [NSString stringWithFormat:@"%lu/%lu", _currentIndex+1, (unsigned long)_models.count];
...
//    _deleteButton.hidden = !_tzImagePickerVc.showSelectBtn;
