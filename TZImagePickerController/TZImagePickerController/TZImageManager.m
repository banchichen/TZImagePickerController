...
- (void)saveVideoWithPath:(NSURL *)path completion:(void (^)(NSError *error))completion {
    [self saveVideoWithPath:path location:nil completion:completion];
}

- (void)saveVideoWithPath:(NSURL *)path location:(CLLocation *)location completion:(void (^)(NSError *error))completion {
    if (iOS9Later) { // 这里有坑... iOS8系统下这个方法保存图片会失败 原来是因为
        [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
            PHAssetResourceCreationOptions *options = [[PHAssetResourceCreationOptions alloc] init];
            options.shouldMoveFile = YES;
            PHAssetCreationRequest *request = [PHAssetCreationRequest creationRequestForAsset];
            [request addResourceWithType:PHAssetResourceTypeVideo fileURL:path options:options];
            if (location) {
                request.location = location;
            }
